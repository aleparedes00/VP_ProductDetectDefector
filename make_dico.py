...
def make_dico_target(path):
    ...
    for i in range(0, len(json_file)):
        for j in range(0, len(json_file[i]['classification'])):
            dico[json_file[i]['classification'][j]['id']] = remove_newline_str(json_file[i]['classification'][j]['text'])
        pass
    pass
    return dico
pass
...
def make_dico_feature(path):
    ...
    for i in range(0, len(json_file)):
        for j in range(0, len(json_file[i]['attributes'])):
            dico[json_file[i]['attributes'][j]['id']] = remove_newline_str(json_file[i]['attributes'][j]['text'])
        pass
    pass
    return dico
pass
